...
struct ContentView: View {
    ...
    @State private var editingEntry: PawnlogEntry?
    ...
    private var emptyState: some View {
        VStack(spacing: 12) {
            ...
            Text("No entries yet")
                ...
                .foregroundStyle(Theme.textPrimary)
            Text("Track items pawned, loan amounts, and redemption deadlines.")
                .font(Theme.captionFont)
                .foregroundStyle(Theme.textSecondary)
    ...
    private func row(for entry: PawnlogEntry) -> some View {
        HStack {
...
struct EntryEditView: View {
    ...
    var onSave: (PawnlogEntry) -> Void

    private let existingID: UUID?

    init(store: Store, existing: PawnlogEntry? = nil, onSave: @escaping (PawnlogEntry) -> Void) {
        self.store = store
        self.onSave = onSave
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        ...
                        let entry = PawnlogEntry(id: existingID ?? UUID(), title: title.isEmpty ? "Untitled" : title, amount: amount, date: date, note: note)
                        onSave(entry)
                        dismiss()
                    }
